...
常用命令:
    ...
    # 指定端口
    python frontend/app_config.py --port 8080

    # 允许局域网访问
    python frontend/app_config.py --host 0.0.0.0 --port 5000

    # 停止已运行的服务
    python frontend/app_config.py --stop
...
@app.route("/")
def index():
    return send_from_directory(HERE / "templates", "index_config.html")
