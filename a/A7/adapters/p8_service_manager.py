...
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent  # a/ 子项目根（2026-09-18：迁移后三层结构坍缩，.gateway.pid 现在在 a/gateway/ 下）
RUNTIME_DIR = PROJECT_ROOT / "data" / "runtime"
RUNTIME_DIR.mkdir(parents=True, exist_ok=True)

# 依赖关系：chat_reply 必须等 gateway 就绪
# 拓扑序：gateway → chat_reply（a/ 子项目不含 web 主流程 P1-P10 web/server.py）
STOP_ORDER = ["chat_reply", "gateway"]
START_ORDER = ["gateway", "chat_reply"]

# 健康检查 URL（None 表示只看进程 PID）
GATEWAY_HEALTHCHECK = "http://127.0.0.1:8787/healthz"
CHAT_REPLY_HEALTHCHECK = None  # 没 HTTP 端口，靠 PID + 日志尾部

# 服务端口（端口预检用；仅 gateway 有）
...
SERVICES: dict[str, dict] = {
    "gateway": {
        ...
        # a/ 子项目版：Node.js Gateway 源码在 a/gateway/(原 openclaw-channel-gateway-standalone/)
        "pid_file": PROJECT_ROOT / "gateway" / ".gateway.pid",
        "log_file": PROJECT_ROOT / "gateway" / "gateway.python.log",
    ...
    # 注:a/ 子项目不含 web 主流程 P1-P10 web/server.py — 已剔除
}
