# Channel Gateway 标准接口

本文说明 `agents/channel_gateway_client.py` 暴露的通道无关接口。业务模块只处理统一事件和发送结果；飞书 `open_id`、`chat_id`、Webhook 校验及 CardKit 细节由适配层和 Gateway 处理。

```text
平台 Webhook → Channel Adapter → 标准事件 → 持久化队列
                                          ↓
业务模块 ← poll / callback / SSE ← Channel Gateway
业务模块 → send / reply → 出站意图 → Channel Adapter → 平台
```
...
也可以用 `feishu_gateway_cli.start_gateway`：
...
参考 `tests/test_feishu_card_transport.py`（卡片发送、回调和原位更新协议）与 `agents/_test_notify_e2e.py`（P8 端到端）。
...
- [飞书接口](FEISHU_API.md)
- [飞书配置教程](FEISHU_CONFIG.md)
- [tests/test_feishu_card_transport.py](../tests/test_feishu_card_transport.py)
