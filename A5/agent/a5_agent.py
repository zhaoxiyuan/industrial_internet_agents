...
class A5Agent:
    ...
    def _parse_output(self, raw: str, wall_time: str, snapshot: Dict) -> List[Dict]:
        ...
        try:
            ...
            raw = raw.strip()
            if raw.startswith("{") and raw.endswith("}"): parsed = json.loads(raw)
            else: return []
        except json.JSONDecodeError: return []
        if not isinstance(parsed, dict) or not parsed.get("is_event"): return []
        sec = int(float(snapshot.get("scenario_time","0s").rstrip("s")))
        pi = parsed.get("person", {}); pid = pi.get("id","?")
        return [{"source":"A5","event_id":parsed.get("event_id",f"A5-{pid}-{int(time.time()*1000)}"),
                 "type":parsed.get("type","未分类"),"person":pi,"second":sec,"wall_time":wall_time,
                 "first_seen":parsed.get("first_seen",wall_time),"last_seen":wall_time,
                 "status":parsed.get("status","ongoing"),"evidence":parsed.get("evidence",{}),
                 "explanation":parsed.get("explanation",""),"note":"A5不判定risk_level,由A6完成"}]
    ...
    def _rule_fallback(self, wall_time: str, snapshot: Dict) -> str:
        ...
        def _has_ongoing(person_id=None, event_type_hint=None):
            ...
            for e in past:
                ...
                if event_type_hint and event_type_hint not in e.get("type",""): continue
    ...
    def _save_raw_event(self, event: Dict, intermediate: List):
        ...
        safe = event["wall_time"].replace(":","").replace("-","").replace(".","_")
        fp = Path(self.raw_event_dir) / f"raw_event_{safe}.json"
        pl = {"wall_time":event["wall_time"],"events":[event],
              "intermediate_steps":[{"action":str(a),"observation":str(o)[:200]} for a,o in (intermediate or [])]}
        fp.parent.mkdir(parents=True, exist_ok=True)
        with open(fp,"w",encoding="utf-8") as f: json.dump(pl, f, ensure_ascii=False, indent=2)
