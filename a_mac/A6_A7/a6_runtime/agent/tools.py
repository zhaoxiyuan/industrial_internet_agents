...
DEFAULT_A6_OUTPUT_DIR = "A6_A7/a6_runtime/logs"
DEFAULT_STATUS_FILE = "A6_A7/a6_runtime/logs/a5_log_status.json"
...
class A5DataTools:
    ...
    def _load_active_violations(self) -> Dict:
        ...
        新格式：active_violations[key] = List[record]（同 key 多条并发活跃记录），
        旧格式（dict 单条）会惰性迁移到 List。
        """
    ...
    def _save_active_violations(self, data: Dict) -> None:
        """保存活跃违规表到磁盘（原子写：tempfile + os.replace）"""
        import os
        import tempfile
        f = Path(ACTIVE_VIOLATIONS_FILE)
        f.parent.mkdir(parents=True, exist_ok=True)
        fd, tmp_path = tempfile.mkstemp(
            prefix="active_violations_", suffix=".json", dir=str(f.parent),
        )
        try:
    ...
    def update_active_violation(
        ...
        同 key 下可以有多个并发活跃记录（按 a6_event_id 区分）；
        若同 a6_event_id 已存在则替换，否则追加。
        """
        active = self._load_active_violations()
        try:
        ...
        for i, rec in enumerate(bucket):
            if rec.get("a6_event_id") == a6_event_id:
                bucket[i] = new_record
                replaced = True
                break
        if not replaced:
    ...
    def close_active_violation(self, key: str, a6_event_id: Optional[str] = None) -> None:
        """关闭活跃违规记录。

        - 不传 a6_event_id：关闭该 key 下所有记录（保持向后兼容）。
        - 传 a6_event_id：只关闭指定记录；若桶为空则删 key。
        """
        active = self._load_active_violations()
        if key not in active:
            return
        if a6_event_id is None:
            for rec in active[key]:
                rec["status"] = "closed"
        else:
            active[key] = [r for r in active[key] if r.get("a6_event_id") != a6_event_id]
            if not active[key]:
                del active[key]
        self._save_active_violations(active)

    def get_active_violation(self, key: str) -> Optional[Dict]:
        """根据 key 查询最新一条活跃违规（按 last_seen 倒序）。"""
        active = self._load_active_violations()
        bucket = active.get(key) or []
        ongoing = [r for r in bucket if r.get("status") == "ongoing"]
        if not ongoing:
            return None
        ongoing.sort(key=lambda r: r.get("last_seen", ""), reverse=True)
        return ongoing[0]

    def get_active_violations_for_key(self, key: str) -> List[Dict]:
        """返回 key 下所有 ongoing 活跃记录，按 last_seen 倒序。

        适用场景：聚合决策时给 LLM 列出所有可能的合并目标（含乱序到达的记录），
        由 LLM 选 last_seen 与当前事件最接近的一条聚合进去。
        """
        active = self._load_active_violations()
        bucket = active.get(key) or []
        ongoing = [r for r in bucket if r.get("status") == "ongoing"]
...
class OutputTools:
    ...
    # ============================================================
    # P7 落盘文件读写工具（供 LLM agent 通过 tool call 调用）
    # 设计目的：消除 process_event() 内部对 load_assessment_with_path 的隐式查找
    #           （该路径与 save_assessment 的写入路径不一致，是聚合失败的根因）。
    #           现在所有读 / 写都走同一目录，路径无歧义。
    # ============================================================

    @staticmethod
    def _parse_wall_time_loose(s: str) -> Optional[datetime]:
        """宽松解析 wall_time：支持 ISO `2026-09-15T08:59:41.906` 与 A5 格式 `2026:09:15T08:59:41.906`。"""
        if not s:
            return None
        s = str(s).strip()
        for fmt in (
    ...
    def query_recent_p7_assessments(
        ...
        """查询 search_dir（或 self.output_dir）下 wall_time ± window_sec 时间窗内的 P7 研判 JSON。

        返回 List[Dict]，每项含完整 P7 JSON 字段，外加 `_filepath` 字段便于 LLM 引用。

        Args:
            wall_time: 锚点时间（ISO 或 A5 格式）
            window_sec: ± 时间窗秒数，默认 60s
            person_id: 可选，按 involved_persons 过滤
            violation_type: 可选，按 event_type 过滤（大小写 / 空格不敏感）
            search_dir: 覆盖默认扫描目录；默认 self.output_dir
        """
        base_dir = Path(search_dir) if search_dir else self.output_dir
        if not base_dir.exists():
            return []

        anchor = self._parse_wall_time_loose(wall_time)
        if anchor is None:
            return []

        # violation_type 过滤：大小写不敏感 + 去所有空白（空格/制表/中文空格）
        # 例 "PPE缺失" / "ppe 缺失" / "PPE  缺失" 都视为同一类
        import re as _re
        vt_norm = _re.sub(r"\s+", "", (violation_type or "").lower()) if violation_type else None

        results: List[Dict[str, Any]] = []
        for fp in base_dir.glob("a6_*.json"):
            try:
            ...
            # 时间窗过滤：以 first_seen 为准，缺失则用 timestamp
            # 严格窗口：与锚点距离 == window_sec 时视为窗外（含边界外推）
            file_t = self._parse_wall_time_loose(
                data.get("first_seen") or data.get("timestamp") or ""
            )
            if file_t is None:
                continue
            if abs((file_t - anchor).total_seconds()) >= window_sec:
                continue

            # 人员过滤
            if person_id:
                persons = data.get("involved_persons") or []
                if person_id not in persons:
                    continue

            # 违规类型过滤（大小写 / 空白都不敏感）
            if vt_norm:
                et = _re.sub(r"\s+", "", (data.get("event_type") or "").lower())
                if et != vt_norm:
                    continue

            data_with_path = dict(data)
            data_with_path["_filepath"] = str(fp)
            results.append(data_with_path)
        return results
    ...
    def write_p7_assessment(
        ...
        """写 P7 研判 JSON 文件（三合一：create / update / delete）。

        Args:
            action: "create" | "update" | "delete"
            a6_event_id: update/delete 时必填；create 可省略（自动生成）
            assessment: create/update 时必填（含完整字段：first_seen / last_seen /
                       event_type / involved_persons / risk_level / risk_basis /
                       suggestions / reasoning / evidence / aggregated_from 等）
            output_dir: 覆盖默认目录；默认 self.output_dir

        Returns:
            Dict，包含 success / action / a6_event_id / filepath / error（失败时）
        """
        ...
        if action == "create":
            if not isinstance(assessment, dict):
                return {"success": False, "action": "create",
                        "error": "create requires assessment (dict)"}
            record = dict(assessment)
            if not record.get("a6_event_id"):
                record["a6_event_id"] = a6_event_id or self._generate_a6_event_id()
            elif not a6_event_id:
                # 用 assessment 里的 a6_event_id
                a6_event_id = record["a6_event_id"]
            record.setdefault("timestamp", datetime.now().isoformat())
            safe_time = record["timestamp"].replace(":", "-").replace(".", "_")
            filepath = base_dir / f"a6_{safe_time}.json"
            with open(filepath, "w", encoding="utf-8") as f:
        ...
        if action == "update":
            if not a6_event_id:
                return {"success": False, "action": "update",
                        "error": "update requires a6_event_id"}
            if not isinstance(assessment, dict):
                return {"success": False, "action": "update",
                        "error": "update requires assessment (dict)"}
            target_fp = None
            for fp in base_dir.glob("a6_*.json"):
                try:
                    data = json.loads(fp.read_text(encoding="utf-8"))
                    if isinstance(data, dict) and data.get("a6_event_id") == a6_event_id:
                        target_fp = fp
                        break
                except (json.JSONDecodeError, IOError):
                    continue
            if target_fp is None:
                return {
                    ...
                    "action": "update",
                    "a6_event_id": a6_event_id,
                    "error": f"未找到 a6_event_id={a6_event_id} 的现有文件",
                }
            record = dict(assessment)
            record["a6_event_id"] = a6_event_id
            record.setdefault("timestamp", datetime.now().isoformat())
            ...
            return {
                ...
                "action": "update",
                "a6_event_id": a6_event_id,
                "filepath": str(target_fp),
            }

        if action == "delete":
            if not a6_event_id:
                return {"success": False, "action": "delete",
                        "error": "delete requires a6_event_id"}
            deleted = False
            for fp in base_dir.glob("a6_*.json"):
                try:
                    data = json.loads(fp.read_text(encoding="utf-8"))
        ...
        return {"success": False, "action": str(action),
                "error": f"unknown action (expect create/update/delete): {action}"}

    def find_assessment_by_event_id(
        self,
        a5_event_id: str,
        search_dir: str = None,
    ) -> Optional[Dict[str, Any]]:
        """扫盘查找 aggregated_from 包含给定 a5_event_id 的 P7 研判文件。

        LLM 通过工具完成 write_p7_assessment 后，Python 用此方法找回刚写入的记录。
        """
        base_dir = Path(search_dir) if search_dir else self.output_dir
        if not base_dir.exists():
            return None
        for fp in base_dir.glob("a6_*.json"):
            ...
            if not isinstance(data, dict):
                continue
            agg = data.get("aggregated_from") or []
            if a5_event_id in agg:
                result = dict(data)
                result["_filepath"] = str(fp)
                return result
        return None
