...
# 路径
# ============================================================
# a/ 子项目版:Node.js Gateway 源码位于 a/gateway/(原 openclaw-channel-gateway-standalone/)。
# Python 客户端 feishu_gateway_cli/ 与 Node.js Gateway 分离(原本嵌在
# openclaw-channel-gateway-standalone/feishu_gateway_cli/),迁移时把 Python 客户端
# 提到 a/feishu_gateway_cli/,Node.js Gateway 降到 a/gateway/。
# 所以 GATEWAY_DIR 不再用 parent.parent,直接定位到 ROOT.sibling("gateway")。

PKG_DIR: Path = Path(__file__).resolve().parent              # a/feishu_gateway_cli/
PROJECT_ROOT: Path = PKG_DIR.parent                          # a/
GATEWAY_DIR: Path = PROJECT_ROOT / "gateway"                  # a/gateway/ (Node.js Gateway 源码)
    ...
    *,
    ...
    if not GATEWAY_DIR.exists():
        raise FileNotFoundError(
            ...
            "请确认 a/gateway/ 已就位(a/ 子项目版 Node.js Gateway 源码目录)。"
        )
