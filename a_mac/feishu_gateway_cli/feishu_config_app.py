...
================================================================================
    ...
    python a/feishu_gateway_cli/feishu_config_app.py
    # 或(模块方式 — 推荐):
    python -m feishu_gateway_cli.feishu_config_app

    # 自定义端口
    python a/feishu_gateway_cli/feishu_config_app.py --port 5003 --host 127.0.0.1
...
# ============================================================
# a/ 子项目版:
# 原项目里 feishu_config_app.py 在 openclaw-channel-gateway-standalone/ 下;迁移到 a/
# 后,Python 客户端提到 a/feishu_gateway_cli/,Node.js Gateway 源码降到 a/gateway/。
# 所以 a/ 既是 STANDALONE_DIR 又是 PROJECT_ROOT(两层结构坍缩为一层)。

PKG_DIR: Path = Path(__file__).resolve().parent              # a/feishu_gateway_cli/
STANDALONE_DIR: Path = PKG_DIR.parent                          # a/(原 standalone 容器)
PROJECT_ROOT: Path = STANDALONE_DIR                            # a/ 同时是 project root
ENV_FILE: Path = PROJECT_ROOT / ".env"                          # a/.env
ENV_BACKUP_FILE: Path = PROJECT_ROOT / ".env.bak"
TEMPLATES_DIR: Path = PKG_DIR / "templates"                    # a/feishu_gateway_cli/templates/

# 把 a/ 加入 sys.path,便于脚本方式运行时 `from feishu_gateway_cli import ...`
if str(STANDALONE_DIR) not in sys.path:
    sys.path.insert(0, str(STANDALONE_DIR))

# Gateway 子进程路径常量（start_gateway 模块已定义）
# 同时支持两种启动方式:
#   python a/feishu_gateway_cli/feishu_config_app.py   (脚本方式 — 项目根在 cwd)
